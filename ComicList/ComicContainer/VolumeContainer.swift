...
public protocol VolumeContainerType {
    ...
    /// Loads the corresponding store for container
    ...
    /// return all the volumes in the container
...
extension VolumeContainer: VolumeContainerType {
    ...
    public func delete(volumeWithIdentifier identifier: Int) -> Observable<Void> {
        return performBackgroundTask { context in
            let fetcRequest = VolumeEntry.fetchRequest(forVolumeWithIdentifier: identifier)
            let entries = try context.fetch(fetcRequest)
    ...
    public func all() -> VolumeResultsType {
        return VolumeResults(fetchRequest: VolumeEntry.defaultFetchRequest(), context: container.viewContext)
    }
